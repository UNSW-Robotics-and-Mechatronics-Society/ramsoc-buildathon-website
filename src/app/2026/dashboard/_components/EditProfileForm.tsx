...
const DEGREE_STAGE_OPTIONS = [
  {
    value: "Pre-penultimate",
    label: "Pre-penultimate (3rd-last year or earlier)",
  },
...
const OPTION_BASE =
  "font-main flex min-h-[44px] cursor-pointer items-center gap-2.5 rounded-lg border px-3 py-2.5 text-sm transition-colors";
const OPTION_ON = "border-lego-yellow bg-lego-yellow/10 text-ink";
const OPTION_OFF =
  "border-white/20 bg-white/5 text-ink-dim hover:border-white/40 hover:text-ink";
...
function CheckboxGroup({
  ...
  return (
    <fieldset className="flex flex-col gap-1.5">
      <legend className="spec-label mb-1.5">{label}</legend>
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        {options.map((opt) => (
          <label
            key={opt.value}
            className={`${OPTION_BASE} ${
              selected.includes(opt.value) ? OPTION_ON : OPTION_OFF
            }`}
          >
            <input
              type="checkbox"
              ...
              className="accent-lego-yellow h-4 w-4 rounded"
            />
            {opt.label}
          </label>
        ))}
      ...
      <div className="flex flex-wrap gap-2">
        {options.map((opt) => (
          <label
            key={opt.value}
            className={`${OPTION_BASE} ${
              value === opt.value ? OPTION_ON : OPTION_OFF
            }`}
          >
            <input
              type="radio"
              ...
              className="accent-lego-yellow h-4 w-4"
            />
            {opt.label}
          </label>
        ))}
...
export default function EditProfileForm({
  ...
  const userType = profile.user_type || "other_uni";
  ...
  const [undergradPostgrad, setUndergradPostgrad] = useState(
    profile.undergrad_postgrad || "",
  );
  const [domesticIntl, setDomesticIntl] = useState(
    profile.domestic_international || "",
  );
  const standardFacultyValues = FACULTY_OPTIONS.filter(
    (f) => f.value !== "Other",
  ).map((f) => f.value);
  const existingFaculties = profile.faculty
    ? profile.faculty.split(", ").filter(Boolean)
    : [];
  const existingOtherFaculty = existingFaculties.find(
    (f) => !standardFacultyValues.includes(f),
  );
  const [selectedFaculties, setSelectedFaculties] = useState<string[]>(
    existingOtherFaculty
      ? [
          ...existingFaculties.filter((f) => f !== existingOtherFaculty),
          "Other",
        ]
      : existingFaculties,
  );
  ...
  const [isRamsocMember, setIsRamsocMember] = useState(
    profile.is_ramsoc_member ?? false,
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <p className="font-blueprint text-ink-dim rounded-lg border border-white/15 bg-white/5 px-3 py-2.5 text-xs uppercase">
        Cohort:{" "}
      ...
      {isUnsw && (
        <fieldset className="flex flex-col gap-2">
          <legend className="spec-label mb-1.5">Memberships</legend>
          <label
            className={`${OPTION_BASE} ${isRamsocMember ? OPTION_ON : OPTION_OFF}`}
          >
            <input
              type="checkbox"
              checked={isRamsocMember}
              onChange={(e) => setIsRamsocMember(e.target.checked)}
              className="accent-lego-yellow h-5 w-5 rounded"
            />
            I am a RAMSoc member
          </label>
          <label
            className={`${OPTION_BASE} ${isArcMember ? OPTION_ON : OPTION_OFF}`}
          >
            <input
              type="checkbox"
              checked={isArcMember}
      ...
      {error && (
        <p role="alert" className="text-lego-red font-main text-sm">
          {error}
        </p>
      )}
